...
L.Model = L.Class.extend({
	...
	handleMessage: function (message) {
		...
		for (const commandId in this._responseCallbacks) {
			...
			if (
				message.command === callbackInfo.command ||
	...
	sendInput: function (input, callback) {
		...
		try {
			...
			if (callback && typeof callback === 'function') {
				...
				// Set timeout to clean up callbacks that never receive a response
				// but don't show an error message to the user
				setTimeout(() => {
					if (this._responseCallbacks[inputId]) {
						// Just clean up the callback without showing an error
						console.log('Cleaning up timed out callback for:', inputId);
						delete this._responseCallbacks[inputId];
					}
				}, 30000); // Increased to 30 second timeout for longer operations
			}
	...
	_onModelResponse: function (e) {
		...
		if (typeof response === 'string' && response.startsWith('modelresponse')) {
			...
			try {
				const jsonStr = response.substring('modelresponse success '.length);
				...
				if (rawText) {
					// Create a simple object with just the conversation field
					parsedResponse = { conversation: rawText };
					console.log('Using raw text as conversation:', parsedResponse);
				}
				return;
			}
		}
	...
	/**
	 * Process a structured response with conversation and snippet
	...
	_processStructuredResponse: function (response) {
		...
		if (result.conversation) {
			...
			let cleanConversation = this._cleanMarkdownText(result.conversation);
		...
		if (result.snippet) {
			...
			let cleanSnippet = this._cleanSnippetText(result.snippet);
			
			// Fire an event for the snippet that the sidebar can listen for
			if (window.app && window.app.map) {
				window.app.map.fire('modelsnippet', {
					snippet: cleanSnippet,
					header: this._extractSnippetHeader(cleanSnippet),
				});
			}
		}
	},
	 ...
	 * @private
	 */
	_cleanMarkdownText: function (text) {
		if (!text) return '';
	...
	/**
	 * Clean snippet text but preserve code formatting
	 * @param {string} text - The text to clean
	 * @returns {string} - The cleaned text
	 * @private
	 */
	_cleanSnippetText: function (text) {
		if (!text) return '';
	...
	formatResponse: function (response) {
		...
		if (typeof response === 'string' && response.startsWith('modelresponse')) {
			...
			const parts = response.split(' ');
			if (parts.length >= 3) {
				// Skip "modelresponse" and status, get the rest
				const jsonStr = parts.slice(2).join(' ');
				
				// Try to parse as JSON to extract conversation
				try {
					const jsonObj = JSON.parse(jsonStr);
					// If it has a conversation field, return that
					if (jsonObj.conversation) {
						return jsonObj.conversation;
					}
					// If it has a result with conversation, return that
					if (jsonObj.result && jsonObj.result.conversation) {
						return jsonObj.result.conversation;
					}
		...
		// Handle structured response with conversation and snippet
		if (response && typeof response === 'object') {
			// Check if the response has the structured format directly
			if (
				response.conversation !== undefined ||
				response.snippet !== undefined
			) {
				// Process the structured response
		...
		// Fallback to string representation, but try to extract conversation if it's JSON
